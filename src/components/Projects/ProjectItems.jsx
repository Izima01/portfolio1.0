...
const ProjectItems = ({ title, image, stack, description, github, live, id }) => {
    ...
    return (
        <article className={`overflow-hidden flex gap-4 items-center flex-col md:flex-row border-b-slate-400 border-b-2 md:border-none pb-3 md:pb-0 ${id % 2 == 0 ? 'md:flex-row-reverse' : ''} w-full`}>
            <motion.img initial={{ visibility: 'hidden', y: 200 }}  whileInView={{ visibility: 'visible', y:0 }} transition={{ type:'spring', delay: 0.5 }} src={image} alt="" className="w-full md:w-7/12 md:h-80" />
            <div className="card_body w-full md:w-5/12 p-2">
                <motion.h3 initial={{ y:50 }}  whileInView={{ y:0 }} transition={{ type: 'spring', delay: 0.5 }} className="text-2xl font-semibold text-black mb-6">{title}</motion.h3>
