  ...
  return (
    <div className="home_data">
        <motion.h1 initial={{ scaleX:0.1, opacity: 0 }} whileInView={{ scaleX: 1, opacity: 1 }} transition={{ duration: 0.75 }} className="font-semibold text-4xl sm:text-[2.75rem] mb-1">
            Izima Obisike
            <motion.img  loading='lazy' animate={{ rotate: [-20, 0, 20, 0, -20] }} transition={{ repeat:'infinite', repeatType: 'loop', duration: 2, type: 'keyframes' }} src={hand} alt="a hand" className='origin-bottom lg:w-9 lg:h-9 w-7 h-7 ml-2 inline-block mb-2' />
        </motion.h1>
        ...
        <a href="#contact" className='group inline-flex gap-2 bg-[#333333] text-white px-7 lg:px-8 py-4 lg:py-5 rounded-2xl font-medium items-center hover:bg-slate-700'>
            Say Hello
            <img loading='lazy' width={32} height={32} src={send} alt="" className='w-6 lg:w-auto h-6 lg:h-auto group-hover:scale-125 group-hover:translate-x-2 group-hover:rotate-45 duration-700' />
        </a>
    </div>
  )
}
